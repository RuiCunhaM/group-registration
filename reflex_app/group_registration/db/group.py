...
def create_pending_group(members):
    with db_cursor() as cur:
        ...
        for member in members:
            cur.execute(
                """insert into group_element(group_id, email, github, invite_id, invite_status) 
                values (%s, %s, %s, %s, %s)""",
                (
                    group_id,
                    member["email"],
...
def get_group_by_invite_id(invite_id):
    with db_cursor() as cur:
        cur.execute(
            """select group_id, email, github, invite_status 
            from group_element  
            where group_id = (select group_id from group_element where invite_id = %s)""",
            (invite_id,),
        )
        return cur.fetchall()
